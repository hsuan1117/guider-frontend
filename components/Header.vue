<template>
    ...
    <TransitionRoot as="template" :show="open">
        <Dialog class="fixed inset-0 flex z-40 lg:hidden" @close="open = false">
            ...
            <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
                ...
                <div class="relative max-w-xs w-full bg-white shadow-xl pb-12 flex flex-col overflow-y-auto">
                        ...
                        <div class="border-b border-gray-200">
                            <TabList class="-mb-px flex px-4 space-x-8">
                                <Tab as="template" v-for="category in navigation.categories" :key="category.name"
                                     v-slot="{ selected }">
                                    <button
                                        :class="[selected ? 'text-primary border-primary' : 'text-gray-900 border-transparent', 'flex-1 whitespace-nowrap py-4 px-1 border-b-2 text-base font-medium']">
                                        {{ category.name }}
                                    </button>
                                </Tab>
                            </TabList>
                    ...
                    <div class="border-t border-gray-200 py-6 px-4 space-y-6">
                        <template v-if="false">
                            <div class="flow-root">
                                <a href="/" class="-m-2 p-2 block font-medium text-gray-900">控制中心</a>
                            </div>
                        </template>
                        <template v-else>
                            <div class="flow-root">
                                <a href="/login" class="-m-2 p-2 block font-medium text-gray-900">登入</a>
                            </div>
                            <div class="flow-root">
                                <a href="/register" class="-m-2 p-2 block font-medium text-gray-900">註冊</a>
                            </div>
                        </template>
                    </div>
                </div>
                ...
                <div class="h-16 flex items-center">
                        ...
                        <div class="h-full flex space-x-8">
                            <Popover v-for="category in navigation.categories" :key="category.name" class="flex"
                                ...
                                <div class="relative flex">
                                    <PopoverButton
                                        :class="[open ? 'border-primary text-primary' : 'border-transparent text-gray-700 hover:text-gray-800', 'relative z-10 flex items-center transition-colors ease-out duration-200 text-sm font-medium border-b-2 -mb-px pt-px']">
                                        {{ category.name }}
                                    </PopoverButton>
                                </div>
                                ...
                                <Transition enter-active-class="transition ease-out duration-200"
                                    ...
                                    <PopoverPanel
                                        class="absolute z-10 top-full inset-x-0 text-sm text-gray-500">
                    ...
                    <div class="ml-auto flex items-center">
                        <div class="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                            <a v-if="user" href="/"
                               class="text-sm font-medium text-gray-700 hover:text-gray-800">控制台</a>
                            <template v-else>
                                <a href="/login" class="text-sm font-medium text-gray-700 hover:text-gray-800">登入</a>
                                <span class="h-6 w-px bg-gray-200" aria-hidden="true"/>
                                <a href="/register"
                                   class="text-sm font-medium text-gray-700 hover:text-gray-800">註冊帳號</a>
                            </template>
                        </div>
...
import {
    ...
    PopoverButton,
    PopoverGroup,
    PopoverPanel, Tab, TabGroup, TabList, TabPanel, TabPanels, TransitionChild,
    TransitionRoot
} from '@headlessui/vue'
